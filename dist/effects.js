...
window.SiteEffects=(()=>{
  ...
  const dots=colors.map((color,index)=>{
    ...
    const dot=document.createElement('div');dot.className='floating-dot'+(big?' floating-dot-large':'');dot.setAttribute('aria-hidden','true');dot.style.background=color;document.body.append(dot);
    return {dot,index,big,color,px:0,py:0,x:Math.random(),y:Math.random(),pulseAt:0,from:Math.random(),to:Math.random(),duration:1700+Math.random()*600,
  ...
  function drawDot(now,w,h,reduced){
    ...
    for(const d of dots){
      ...
      dot.style.left=d.px+'px';dot.style.top=d.py+'px';
    }
    ...
    for(const d of dots.slice(0,-1)){
      const delta=Math.hypot(large.px-d.px,large.py-d.py);
