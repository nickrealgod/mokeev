...
const wave={performance:{now:()=>wave.now},now:1000,waveUntil:0,queuedWave:null,lastScrollFamily:'white',reduce:{matches:false},items:Array.from({length:10},()=>({style:'white',variants:{white:{},black:{}}})),commonFamilies:()=>['white','black'],pick:a=>a[0],options:()=>['black'],gradientSet:()=>[],apply:(item,style)=>item.style=style};
